...
| File | Holds |
| --- | --- |
| `src/data/library.js` | `LIBRARY` catalog, `GROUPS`/`GROUP_ALIASES`, `BUILTIN_PRESETS`, `resolveEntry`/`withUids`, `GET_READY`, the presets storage key |
...
- **Data model**: `LIBRARY` is the fixed catalog of timed exercises (id, name, duration, `group`, cue, desc, optional `alt` search aliases), spanning the six `GROUPS` — Core, Upper Body, Lower Body, Full Body, Cardio, Mobility. Everything in it is holdable or repeatable for a stretch of time, since the timer counts seconds and not reps. Presets (built-in `BUILTIN_PRESETS` plus user-saved ones) store exercise entries that are either a plain id string or `{ id, duration }` when the duration was customized (resolved via `resolveEntry`); the working workout state wraps each entry with a generated `uid` (via `withUids`) so duplicates can be reordered/removed independently.
